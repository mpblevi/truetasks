...
function toEmail(u) {
  return u.toLowerCase().trim() + DOMAIN;
}
          ...
          {opcoes.map((o, i) => (
            <button
              ...
              style={{
                ...
                textAlign: "left",
                cursor: "pointer",
                borderBottom: i < opcoes.length - 1 ? "1px solid #f1f5f9" : "none",
                fontFamily: "'Lato', sans-serif",
              }}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 400, padding: 20 }}>
      <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 16, padding: 32, width: "100%", maxWidth: 440, boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
        ...
        <div style={{ fontSize: 13, color: "#64748b", marginBottom: 20 }}>
          Tarefa: <strong style={{ color: "#1e293b" }}>{tarefa.cliente} — {tarefa.tipo}</strong>
        </div>

        {cfg.tipo === "select_profile" && (
          <div>
            <label style={LABEL}>{cfg.campo}</label>
        ...
        {cfg.tipo === "date_duplo" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div>
              ...
              <input type="date" value={valor} onChange={e => setValor(e.target.value)} style={INPUT} defaultValue={tarefa.prazo_interno} />
            </div>
            <div>
              <label style={LABEL}>Prazo Legal</label>
              <input type="date" value={valor2} onChange={e => setValor2(e.target.value)} style={INPUT} defaultValue={tarefa.prazo_legal} />
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 24, justifyContent: "flex-end" }}>
          <button onClick={onFechar} style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 9, color: "#64748b", padding: "10px 20px", fontSize: 14, cursor: "pointer" }}>
            Cancelar
          </button>
          <button onClick={salvar} disabled={loading} style={{ ...BTN_PRIMARY, width: "auto", opacity: loading ? 0.7 : 1 }}>
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4f8", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Lato', sans-serif" }}>
      ...
      <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 18, padding: 40, width: "100%", maxWidth: 400, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <button onClick={handleLogin} disabled={loading} style={{ ...BTN_PRIMARY, marginTop: 8, opacity: loading ? 0.7 : 1 }}>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }}>
      <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 16, padding: 32, width: "100%", maxWidth: 640, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 18, fontWeight: 800, color: "#1a56db" }}>
            Gerenciar Clientes
          </div>
          <button onClick={onFechar} style={{ background: "none", border: "none", color: "#94a3b8", fontSize: 22, cursor: "pointer" }}>
            ×
          </button>
        </div>

        {msg && (
          <div style={{ background: "#dcfce7", border: "1px solid #22c55e", borderRadius: 8, padding: "10px 14px", color: "#15803d", fontSize: 13, marginBottom: 16 }}>
            {msg}
          </div>
        )}
            ...
            return (
              <div key={c.id} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button
                  ...
                  style={{ background: "#fee2e2", border: "none", borderRadius: 8, color: "#dc2626", padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}
                >
                  Remover
                </button>
              </div>
        ...
        <button onClick={() => { setModalNovo(true); setErro(""); setMsg(""); }} style={{ ...BTN_PRIMARY }}>
        ...
        {modalNovo && (
          <div style={{ marginTop: 24, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div>
                <label style={LABEL}>Responsável padrão</label>
                <select value={form.responsavel_id} onChange={e => setForm(f => ({ ...f, responsavel_id: e.target.value }))} style={INPUT}>
                  <option value="">Selecione...</option>
                  {profiles.map(p => (
                    <option key={p.id} value={p.id}>
                      {p.nome}
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setModalNovo(false)} style={{ flex: 1, background: "white", border: "1px solid #e2e8f0", borderRadius: 9, color: "#64748b", padding: "10px", fontSize: 14, cursor: "pointer" }}>
                  Cancelar
                </button>
                <button
                  onClick={criarCliente}
                  disabled={loading}
                  style={{ ...BTN_PRIMARY, flex: 2, width: "auto", opacity: loading ? 0.7 : 1 }}
                >
...
function PainelUsuarios({ profiles, onAtualizar, onFechar }) {
  ...
  async function criarUsuario() {
    ...
    const { data, error } = await supabase.auth.signUp({
      email: toEmail(form.usuario),
      password: form.senha,
    });

    if (error) {
      setErro("Erro: " + (error.message.includes("already") ? "Usuário já existe." : error.message));
      setLoading(false);
      return;
    }
            ...
            <div key={p.id} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>
                  ...
                  <span style={{ marginLeft: 12, color: p.cargo === "admin" ? "#d97706" : "#64748b", textTransform: "uppercase", letterSpacing: 1 }}>
              ...
              {p.cargo !== "admin" && (
                <button
                  onClick={() => excluirUsuario(p.id, p.nome)}
                  style={{ background: "#fee2e2", border: "none", borderRadius: 8, color: "#dc2626", padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}
                >
                  Remover
                </button>
          ...
          <div style={{ marginTop: 24, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div>
                ...
                <select value={form.cargo} onChange={e => setForm(f => ({ ...f, cargo: e.target.value }))} style={INPUT}>
  ...
  async function replicar() {
    ...
    const novas = selecionados.map(clienteId => {
      ...
      return {
        ...
        revisor_id: tarefa.revisor_id,
        revisor_nome: tarefa.revisor_nome,
      ...
      <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 16, padding: 32, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 32px rgba(0,0,0,0.15)" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
          ...
          <button
            ...
            style={{ background: "#dbeafe", border: "none", borderRadius: 8, color: "#1d4ed8", padding: "10px 14px", fontSize: 13, cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap" }}
          >
            Selecionar todos
          </button>
        </div>
...
export default function App() {
  ...
  const [showModalAcaoLote, setShowModalAcaoLote] = useState(false);
  ...
  function abrirEditar(t) {
    ...
    setForm({
      ...
      prazo_interno: t.prazo_interno || today(),
      prazo_legal: t.prazo_legal || today(),
  ...
  async function salvar() {
    ...
    const payload = {
      ...
      cnpj_cliente: clienteObj?.cnpj || "",
      tipo: form.tipo,
      competencia: form.competencia,
      prazo_interno: form.prazo_interno,
      prazo_legal: form.prazo_legal,
      prazo: form.prazo_interno,
      responsavel_id: form.responsavel_id,
      responsavel_nome: respNome,
      revisor_id: form.revisor_id || null,
      revisor_nome: revNome,
  ...
  async function excluirSelecionadas() {
    ...
    setShowModalAcaoLote(false);
  }

  async function handleAcaoLote() {
    if (acaoLote === "delete_selected") {
  ...
  async function gerarProximoMes() {
    ...
    if (recorrentes.length === 0) {
      setMsgRecorrente("Nenhuma tarefa marcada como recorrente.");
    ...
    const novas = recorrentes.map(t => ({
      cliente: t.cliente,
      cnpj_cliente: t.cnpj_cliente,
      tipo: t.tipo,
      competencia: addMonthsComp(t.competencia, 1),
  ...
  function toggleSelectTarefa(id) {
    ...
    <div style={{ minHeight: "100vh", width: "100%", background: "#f0f4f8", fontFamily: "'Lato', sans-serif", color: "#1e293b" }}>
      ...
      {showModalAcaoLote && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 400, padding: 20 }}>
          <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 16, padding: 32, width: "100%", maxWidth: 440, boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 17, fontWeight: 800, color: "#1a56db" }}>
                Ações em Lote
              </div>
              <button onClick={() => setShowModalAcaoLote(false)} style={{ background: "none", border: "none", color: "#94a3b8", fontSize: 22, cursor: "pointer" }}>
                ×
              </button>
            </div>

            <div style={{ fontSize: 13, color: "#64748b", marginBottom: 20 }}>
              {selecionadas.size} tarefa(s) selecionada(s)
            </div>

            <div style={{ marginBottom: 20 }}>
              <label style={LABEL}>Selecione uma ação</label>
              <select value={acaoLote} onChange={e => setAcaoLote(e.target.value)} style={INPUT}>
                <option value="">— Selecione —</option>
                <option value="delete_selected">Excluir Selecionadas</option>
              </select>
            </div>

            <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
              <button onClick={() => setShowModalAcaoLote(false)} style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 9, color: "#64748b", padding: "10px 20px", fontSize: 14, cursor: "pointer" }}>
                Cancelar
              </button>
              <button
                onClick={handleAcaoLote}
                disabled={!acaoLote}
                style={{ ...BTN_PRIMARY, width: "auto", opacity: !acaoLote ? 0.5 : 1 }}
              >
                Aplicar
              </button>
            </div>
      ...
      {showConfirmDelete && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 400, padding: 20 }}>
          <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 16, padding: 32, width: "100%", maxWidth: 440, boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
            <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 17, fontWeight: 800, color: "#dc2626", marginBottom: 16 }}>
              ⚠️ Confirmar Exclusão
            </div>

            <p style={{ fontSize: 14, color: "#64748b", marginBottom: 24 }}>
              Tem certeza que deseja excluir {selecionadas.size} tarefa(s)? Esta ação não pode ser desfeita.
            </p>

            <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
              <button onClick={() => setShowConfirmDelete(false)} style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 9, color: "#64748b", padding: "10px 20px", fontSize: 14, cursor: "pointer" }}>
                Cancelar
              </button>
              <button
                onClick={excluirSelecionadas}
                style={{ background: "#fee2e2", border: "1px solid #fca5a5", borderRadius: 9, color: "#dc2626", padding: "10px 20px", fontSize: 14, cursor: "pointer", fontWeight: 600 }}
              >
                Sim, Excluir
              </button>
            </div>
      ...
      <div style={{ background: "#1a56db", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 11, color: isAdmin ? "#fde68a" : "rgba(255,255,255,0.7)", textTransform: "uppercase", letterSpacing: 1 }}>
          ...
          {isAdmin && (
            <>
              <button
                onClick={() => setPainelClientes(true)}
                style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, color: "white", padding: "8px 14px", fontSize: 13, cursor: "pointer", fontWeight: 600 }}
              >
                Clientes
              </button>

              <button
                onClick={() => setPainelUsuarios(true)}
                style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, color: "white", padding: "8p
