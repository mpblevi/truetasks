...
const DOMAIN = "@truetasks.app"; // e-mail fictício, usuário nunca vê isso
...
function isAtrasado(prazo, status) {
...
const btnPrimary = {
  ...
  fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", width: "100%",
};
...
function Logo({ size = 24 }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <span style={{ fontFamily: "'Syne', sans-serif", fontSize: size, fontWeight: 800, color: "#e2e8f0", letterSpacing: -0.5 }}>
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080f18", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ background: "#0d1b2a", border: "1px solid #1e3a5f", borderRadius: 18, padding: 40, width: "100%", maxWidth: 400 }}>
        ...
        {erro && (
          <div style={{ background: "#2a1e1e", border: "1px solid #ef4444", borderRadius: 8, padding: "10px 14px", color: "#fca5a5", fontSize: 13, marginBottom: 20 }}>
            ⚠️ {erro}
          </div>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            ...
            <input value={usuario} onChange={e => setUsuario(e.target.value)} placeholder="ex: 001 ou ana" style={inputStyle}
          ...
          <button onClick={handleLogin} disabled={loading} style={{ ...btnPrimary, marginTop: 8, opacity: loading ? 0.7 : 1 }}>
...
function PainelUsuarios({ profiles, onAtualizar, onFechar }) {
  ...
  async function criarUsuario() {
    ...
    setLoading(true); setErro("");
    const email = toEmail(form.usuario);
    const { data, error } = await supabase.auth.signUp({ email, password: form.senha });
    if (error) { setErro("Erro: " + (error.message.includes("already") ? "Esse usuário já existe." : error.message)); setLoading(false); return; }
    if (data.user) {
      await supabase.from("profiles").insert({ id: data.user.id, nome: form.nome, cargo: form.cargo, usuario: form.usuario });
      setMsg(`Usuário "${form.usuario}" criado com sucesso!`);
  ...
  async function excluirUsuario(id, nome) {
    if (!window.confirm(`Remover o usuário "${nome}"?`)) return;
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }}>
      <div style={{ background: "#0d1b2a", border: "1px solid #1e3a5f", borderRadius: 16, padding: 32, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: "#0ea5e9" }}>👥 Gerenciar Usuários</div>
            ...
            <div key={p.id} style={{ background: "#0a1220", border: "1px solid #1e3a5f", borderRadius: 10, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: "#334155", marginTop: 2 }}>
                  ...
                  <span style={{ marginLeft: 12, color: p.cargo === "admin" ? "#fbbf24" : "#475569", textTransform: "uppercase", letterSpacing: 1 }}>
                    {p.cargo === "admin" ? "👑 Admin" : "👤 Colaborador"}
                  </span>
                </div>
              </div>
              {p.cargo !== "admin" && (
                <button onClick={() => excluirUsuario(p.id, p.nome)} style={{ background: "#2a1e1e", border: "none", borderRadius: 8, color: "#f87171", padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                  Remover
                </button>
              )}
            </div>
        ...
        <button onClick={() => { setModalNovo(true); setErro(""); setMsg(""); }} style={{ ...btnPrimary }}>
          ...
          <div style={{ marginTop: 24, background: "#0a1220", border: "1px solid #1e3a5f", borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                ...
                <input value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} placeholder="Ex: Ana Lima" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Usuário (para login)</label>
                <input value={form.usuario} onChange={e => setForm(f => ({ ...f, usuario: e.target.value.replace(/\s/g, "") }))} placeholder="Ex: 001 ou ana" style={inputStyle} />
                <div style={{ fontSize: 11, color: "#334155", marginTop: 4 }}>Sem espaços. Ex: 001, ana, carlos</div>
              </div>
              <div>
                <label style={labelStyle}>Senha inicial</label>
                <input type="password" value={form.senha} onChange={e => setForm(f => ({ ...f, senha: e.target.value }))} placeholder="Mínimo 6 caracteres" style={inputStyle} />
              </div>
              <div>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                ...
                <button onClick={criarUsuario} disabled={loading} style={{ ...btnPrimary, flex: 2, width: "auto", opacity: loading ? 0.7 : 1 }}>
...
export default function App() {
  ...
  const [form, setForm] = useState({ cliente: "", tipo: "IRPF", prazo: today(), responsavel_id: "", status: "A Fazer", prioridade: "Média", obs: "" });
  ...
  async function carregarTarefas() {
    let query = supabase.from("tarefas").select("*").order("prazo");
  ...
  function abrirNova() {
    setEditando(null);
    setForm({ cliente: "", tipo: "IRPF", prazo: today(), responsavel_id: user.id, status: "A Fazer", prioridade: "Média", obs: "" });
    setModal(true);
  }
  function abrirEditar(t) {
    setEditando(t.id);
    setForm({ cliente: t.cliente, tipo: t.tipo, prazo: t.prazo, responsavel_id: t.responsavel_id, status: t.status, prioridade: t.prioridade, obs: t.obs || "" });
  ...
  async function salvar() {
    ...
    const payload = { ...form, responsavel_nome: respNome, criado_por: user.id };
    if (editando) {
  ...
  const tarefasComStatus = tarefas.map(t => ({
    ...t, status: isAtrasado(t.prazo, t.status) ? "Atrasado" : t.status
  }));
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#080f18", display: "flex", alignItems: "center", justifyContent: "center", color: "#475569", fontFamily: "'DM Sans', sans-serif" }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Syne:wght@700;800&display=swap" rel="stylesheet" />
      Carregando...
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080f18", fontFamily: "'DM Sans', sans-serif", color: "#e2e8f0" }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Syne:wght@700;800&display=swap" rel="stylesheet" />
      ...
      <div style={{ background: "linear-gradient(135deg, #0d1b2a, #0a1628)", borderBottom: "1px solid #1e3a5f", padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 11, color: profile.cargo === "admin" ? "#fbbf24" : "#475569", textTransform: "uppercase", letterSpacing: 1 }}>
              {profile.cargo === "admin" ? "👑 Admin" : "👤 Colaborador"}
            </div>
          </div>
          {isAdmin && (
            <>
              <button onClick={() => setPainelUsuarios(true)} style={{ background: "#1e3a5f", border: "1px solid #2a4a7f", borderRadius: 9, color: "#93c5fd", padding: "9px 16px", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>
                👥 Usuários
              </button>
              <button onClick={abrirNova} style={{ background: "linear-gradient(135deg, #0ea5e9, #0284c7)", border: "none", borderRadius: 9, color: "#fff", padding: "9px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
                + Nova Tarefa
              </button>
            </>
          )}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px" }}>
          ...
          {Object.entries(stats).map(([s, n]) => {
            ...
            return (
              <div key={s} onClick={() => setFiltroStatus(filtroStatus === s ? "Todos" : s)}
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: st.border, fontFamily: "'Syne', sans-serif" }}>{n}</div>
        ...
        {/* TABELA */}
        <div style={{ background: "#0d1b2a", border: "1px solid #1e3a5f", borderRadius: 14, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#0a1628", borderBottom: "1px solid #1e3a5f" }}>
                {["Prioridade", "Cliente", "Tipo", "Prazo", "Responsável", "Status", "Ações"].map(h => (
                  <th key={h} style={{ padding: "13px 16px", textAlign: "left", fontSize: 11, color: "#475569", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtradas.length === 0 && (
                <tr><td colSpan={7} style={{ padding: 40, textAlign: "center", color: "#334155" }}>Nenhuma tarefa encontrada.</td></tr>
              )}
              {filtradas.map((t, i) => {
                ...
                return (
                  <tr key={t.id} style={{ borderBottom: "1px solid #111f30", background: i % 2 === 0 ? "transparent" : "#0a1220" }}
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <span style={{ color: pr.color, fontSize: 16 }}>●</span>
                      <span style={{ fontSize: 11, color: pr.color, marginLeft: 6 }}>{t.prioridade}</span>
                    </td>
                    <td style={{ padding: "13px 16px", fontWeight: 600, fontSize: 14, color: "#cbd5e1" }}>{t.cliente}</td>
                    <td style={{ padding: "13px 16px" }}>
                      <span style={{ background: "#0f2a4a", border: "1px solid #1e3a5f", borderRadius: 6, padding: "3px 10px", fontSize: 12, color: "#60a5fa", fontWeight: 600 }}>{t.tipo}</span>
                    </td>
                    <td style={{ padding: "13px 16px", fontSize: 13, color: t.status === "Atrasado" ? "#f87171" : "#94a3b8", fontWeight: t.status === "Atrasado" ? 700 : 400 }}>
                      {t.prazo?.split("-").reverse().join("/")}
                    </td>
                    <td style={{ padding: "13px 16px", fontSize: 13, color: "#94a3b8" }}>{t.responsavel_nome}</td>
                    <td style={{ padding: "13px 16px" }}>
                      <span style={{ background: st.bg, border: `1px solid ${st.border}`, borderRadius: 20, padding: "4px 12px", fontSize: 12, color: st.text, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6 }}>
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setDetalhes(t)} style={{ background: "#1e3a5f", border: "none", borderRadius: 7, color: "#93c5fd", padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>Ver</button>
                        {(isAdmin || t.responsavel_id === user.id) && (
                          <button onClick={() => abrirEditar(t)} style={{ background: "#1a3a2a", border: "none", borderRadius: 7, color: "#86efac", padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>Editar</button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {modal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
          <div style={{ background: "#0d1b2a", border: "1px solid #1e3a5f", borderRadius: 16, padding: 32, width: "100%", maxWidth: 520 }}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: "#0ea5e9", marginBottom: 24 }}>
              {editando ? "✏️ Editar Tarefa" : "➕ Nova Tarefa"}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div style={{ gridColumn: "1/-1" }}>
                <label style={labelStyle}>Cliente *</label>
                <input value={form.cliente} onChange={e => setForm(f => ({ ...f, cliente: e.target.value }))} placeholder="Nome do cliente ou empresa" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Tipo de Obrigação</label>
              ...
              <div>
                ...
                <select value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))} style={inputStyle}>
                  {STATUS_LIST.filter(s => s !== "Atrasado").map(s => <option key={s}>{s}</option>)}
                </select>
              </div>
              <div style={{ gridColumn: "1/-1" }}>
                <label style={labelStyle}>Observações</label>
                <textarea value={form.obs} onChange={e => setForm(f => ({ ...f, obs: e.target.value }))} placeholder="Anotações adicionais..." style={{ ...inputStyle, height: 80, resize: "vertical" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: 12, marginTop: 24, justifyContent: "flex-end" }}>
              ...
              <button onClick={salvar} disabled={formLoading} style={{ background: "linear-gradient(135deg, #0ea5e9, #0284c7)", border: "none", borderRadius: 9, color: "#fff", padding: "10px 28px", fontSize: 14, fontWeight: 700, cursor: "pointer", opacity: formLoading ? 0.7 : 1 }}>
                {formLoading ? "Salvando..." : editando ? "Salvar Alterações" : "Criar Tarefa"}
              </button>
            </div>
          </div>
        </div>
      ...
      {detalhes && (() => {
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
            <div style={{ background: "#0d1b2a", border: `1px solid ${st.border}`, borderRadius: 16, padding: 32, width: "100%", maxWidth: 460 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: "#e2e8f0" }}>{t.cliente}</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {[["Tipo", t.tipo], ["Prazo", t.prazo?.split("-").reverse().join("/")], ["Responsável", t.responsavel_nome], ["Prioridade", <span style={{ color: pr.color }}>{t.prioridade}</span>]].map(([k, v]) => (
                  <div key={k} style={{ background: "#0a1220", borderRadius: 10, padding: "12px 16px" }}>
                    <div style={{ fontSize: 11, color: "#475569", fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>{k}</div>
                    <div style={{ fontSize: 14, color: "#cbd5e1", fontWeight: 600 }}>{v}</div>
                  </div>
                ))}
              </div>
              {podeEditar && (
                    ...
                    {STATUS_LIST.filter(s => s !== "Atrasado").map(s => {
                      ...
                      return (
                        <button key={s} onClick={() => { alterarStatus(t.id, s); setDetalhes({ ...t, status: s }); }}
                          style={{ background: t.status === s ? ss.bg : "transparent", border: `1px solid ${t.status === s ? ss.border : "#1e3a5f"}`, borderRadius: 20, color: t.status === s ? ss.text : "#475569", padding: "5px 14px", fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                          {s}
                        </button>
                      );
                    })}
              ...
              {podeEditar && (
                <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
                  ...
                  {isAdmin && (
                    <button onClick={() => excluir(t.id)} style={{ background: "#2a1e1e", border: "none", borderRadius: 9, color: "#f87171", padding: "10px 18px", fontSize: 14, cursor: "pointer", fontWeight: 600 }}>🗑️</button>
                  )}
                </div>
              )}
            </div>
